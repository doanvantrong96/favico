    ...
    'columns' => [
        ...
        [
            ...
            'value' => function($data){
                ...
                $html .= '<p><b>Thời gian từ chối duyệt</b>: ' . date('H:i d/m/Y',strtotime($data->date_reject)) . ' <span data-toggle="tooltip" class="fal fa-info-circle" style="color: #fff; font-size: 15px; background-color: #886ab5; border-radius: 50%;" title="Lý do: ' . str_replace('"',"'", $data->reason) . '"></span></p>';
        ...
        [
            ...
            'value' => function($data){
                $html = '';
                $modelUserReject = null;
                $modelUserEdit = null;
                $modelUserCreate = Employee::findOne($data->author);
                if( $data->user_reject ){
                    $modelUserReject = Employee::findOne($data->user_reject);
                }
                if( $data->user_update ){
                    $modelUserEdit = Employee::findOne($data->user_update);
                }
                $html .= '<p><b>Người từ chối duyệt</b>: ' . ($modelUserReject ? $modelUserReject->fullname : '---') . '</p>';
        ...
        'contentOptions' => ['class'=>'text-center td_action','style'=>'width:160px'],
            ...
            'buttons' => [
                ...
                'request_approve' => function ($model, $url) use ( $controller){
                    ...
                    if( CommonController::checkAccess($action) )
                        return '<a data-toggle="tooltip" class="btn-action request-approve" data-placement="bottom" title="Gửi xét duyệt" href="' . $action . '?id=' . $url->id . '">Gửi xét duyệt bài viết</a>';
                    return '';
                },
                'remove' => function ($model, $url) use ( $controller){
                    $action = '/' . $controller . '/delete-reject';
                    if( CommonController::checkAccess($action) )
                        return '<a data-toggle="tooltip" class="btn-action delete" data-placement="bottom" title="Xoá bài viết" href="' . $action . '?id=' . $url->id . '">Xoá bài viết</a>';
                    return '';
                }
            ],
        ],
