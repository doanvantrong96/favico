    ...
    'columns' => [
        ...
        [
            ...
            'value' => function($data){
                ...
                $html .= '<p data-toggle="tooltip"  data-placement="bottom" title="Thời gian duyệt bài"><b>Thời gian duyệt</b>: ' . date('H:i d/m/Y',strtotime($data->date_approved)) . '</p>';
        ...
        [
            ...
            'value' => function($data){
                $html = '';
                $modelUserApprove = null;
                $modelUserEdit = null;
                $modelUserCreate = Employee::findOne($data->author);
                if( $data->user_approve ){
                    $modelUserApprove = Employee::findOne($data->user_approve);
                }
                if( $data->user_update ){
                    $modelUserEdit = Employee::findOne($data->user_update);
                }
                $html .= '<p><b>Người duyệt</b>: ' . ($modelUserApprove ? $modelUserApprove->fullname : '---') . '</p>';
        ...
        'contentOptions' => ['class'=>'text-center td_action','style'=>'width:160px'],
            ...
            'buttons' => [
                ...
                'reject' => function ($model, $url) use ( $controller){
                    ...
                    if( CommonController::checkAccess($action) )
                        return '<a data-toggle="tooltip" data-placement="bottom" title="Từ chối xuất bản" class="btn-action reject-approve" href="' . $action . '?id=' . $url->id . '">Từ chối xuất bản</a>';
                    return '';
                },
                'remove' => function ($model, $url) use ( $controller){
                    $action = '/' . $controller . '/delete-approve';
                    if( CommonController::checkAccess($action) )
