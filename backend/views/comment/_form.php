            ...
            <div class="panel" style="width:100%">
                ...
                <div class="panel-container show">
                    <div class="panel-content row">
                        ...
                        <div class="col-md-6">
                            <?= $form->field($model, 'type')->dropDownList(Yii::$app->params['commentTypeList']) ?>
                        </div>
                        <div class="col-md-6">
                            <?= $form->field($model, 'position')->textInput(['maxlength' => 5])->label('Thứ tự hiển thị') ?>
                        </div>
                        <div class="col-md-6">
                            <?= $form->field($model, 'rate')->textInput(['maxlength' => 5])->label('Số sao đánh giá') ?>
                        </div>
                        <div class="col-md-6">
                            <?= $form->field($model, 'author')->textInput(['maxlength' => true,'input-set'=>'.set-unicode','class'=>'form-control remove-unicode'])->label('Tên người bình luận') ?>
                        </div>
                        <div class="col-md-6">
                            <?= $form->field($model, 'product_id')->textInput(['maxlength' => 50])->label('Id sản phẩm') ?>
                        </div>
                        <div class="col-lg-12">
                            <div class="form-group mb-0">
                                <label class="control-label">Ảnh nền</label>
                                <div class="custom-file">
                                    <input type="file" name="avatar" accept="image/*" class="custom-file-input file-upload-ajax" data-folder="images/comment" id="imgUpload">
                                    <label class="custom-file-label" for="imgUpload"><?= $model->avatar != '' ? $model->avatar : 'Chọn ảnh' ?></label>
                                </div>
                                <img class="img-preview" src="<?= $model->avatar ?>" style="<?= $model->avatar != '' ? '' : 'display:none' ?>" />
                                <?= $form->field($model, 'avatar')->hiddenInput(['class'=>'input-hidden-value'])->label(false); ?>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <?= $form->field($model, 'content')->textarea(['class'=>'form-control','maxlength'=>300,'style'=>'height: 60px;']); ?>
                        </div>
                    </div>
                </div>
            </div>
