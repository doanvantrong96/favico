...
$(document).on('click','.label_move',function (){
    ...
    if (items && items.length)
        var url = (target == 'assigned') ? '/assignment/revoke?id=' + idUser : '/assignment/assign?id=' + idUser; 
...
$(document).on('change','.select_type select',function (){
    ...
    $('.btn_new_account').attr('href','/assignment/create?type=' + _this.val());
});
$('.btn-change-status').click(function(){
    ...
    if( list_input_checked.length > 0 ){
        ...
        $.post('/assignment/changestatus', {data: listData }, function (r) {
...
$('.btn_banned').click(function(){
    ...
    if( confirm( text_confirm ) ){
        ...
        $.post('/assignment/changestatus', {data: [userid], type_account : $('.select_type select').val()}, function (r) {
...
function search(target) {
    ...
    $.each(_opts.items[target], function (name, group) {
        if (name.indexOf(q) >= 0) {
            ...
            $list.append('<li><label class="label_move" data-target="' + target + '" data-name="' + name + '">' + nameShow  + '</label> <a style="text-decoration:none !important;color:#0096D7" href="/role/view?id=' + (name.replace(/\ /g,'+')) + '" target="_blank" class="fal fa-search"></a></li>');
        }
    });
}
