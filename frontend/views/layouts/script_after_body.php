<?php
    ...
    /*
    if( $_SERVER['SERVER_NAME'] == Yii::$app->params['website'] ):
?>
...
<?php
    ...
    */
    ...
    if( isset($params['log']) && isset($params['log']['enable']) && $params['log']['enable'] && !isBot($browserInfo['userAgent']) ){
      // $session            = Yii::$app->session;
      // if ( !$session->isActive) { $session->open(); }
      //$data = session_id + id + type + category + user_agent + time_expire
      $log                = $params['log'];
      $str_data           = Yii::$app->session->getId() . '#_#' . $log['id'] . '#_#' . $log['type'] . '#_#' . (isset($log['cate_id']) ? $log['cate_id'] : "NA") . '#_#' . $browserInfo['userAgent'] . '#_#' . (time() + 10);
      $data               = CommonController::encryptDecrypt($str_data);
?>
<script async defer type="text/javascript" src="/js/tracking.js"></script>
<script type="text/javascript">
$(window).load(function(){
  var interactor = new Interactor('<?= Url::to(['site/tracking']) ?>', {
    interactions: false,
    conversions: false,
    data: '<?= $data ?>',
